...
function TransactionFormDialog({ open, onOpenChange, transaction, onSubmit }) {
  ...
  const subcategories = form.category
    ? getSubcategories(form.category, customSubcategories, customCategories)
    : [];
  ...
  const validate = useCallback((data) => {
    ...
    if (data.category && data.category !== "transfer" && !data.subcategory)
      errs.subcategory = "Please select a subcategory";
    if (!data.account) errs.account = "Please select an account";
    if (
      (data.category === "transfer" || data.category === "investments") &&
      !data.toAccount
    )
      errs.toAccount = "Please select destination account";
    if (
      (data.category === "transfer" || data.category === "investments") &&
      data.toAccount &&
      data.account === data.toAccount
    )
      ...
      next =
        value === "transfer"
          ...
          : {
              ...
              category: value,
              subcategory: "",
              toAccount: value === "investments" ? form.toAccount : "",
            };
    } else {
  ...
  const handleSubmit = (e) => {
    ...
    if (form.category !== "transfer" && form.category !== "investments")
          ...
          <div className="flex flex-col gap-6 px-6 py-5">
            ...
            <div className="flex flex-col gap-2">
              <Label
                className={cn(
                  ...
                  errors.amount && touched.amount && "text-destructive",
                )}
              >
                Amount
              </Label>
            ...
            <div className="flex flex-col gap-6">
              ...
              {!isTransfer && (
                <div className="flex flex-col gap-2">
                  ...
                  <FieldError
                    message={touched.subcategory && errors.subcategory}
                  />
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-6">
              <div className="flex flex-col gap-2">
                <Label
                  className={
                    errors.account && touched.account ? "text-destructive" : ""
                  }
                >
              ...
              {showToAccount && (
                <div className="flex flex-col gap-2">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => set("toAccount", v)}
                  >
                    <SelectTrigger
                      className={cn(
                        "h-10",
              ...
              <div className="flex flex-col gap-2">
                <Label
                  className={
                    errors.date && touched.date ? "text-destructive" : ""
                  }
                >
                  Date
                </Label>
...
function TransactionItem({ transaction, onEdit, onDelete }) {
  ...
  const toAccount = transaction.toAccount
    ? accounts.find((a) => a.id === transaction.toAccount)
    : null;
  ...
  return (
    <div className="flex items-start sm:items-center gap-3 p-3 rounded-lg hover:bg-secondary/30 transition-colors group">
      <div
        ...
        style={{
          backgroundColor: `${cat?.hex || "#666"}20`,
          color: cat?.hex || "#666",
        }}
      >
        {isTransfer ? (
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
          ...
          {account && (
            <>
              ...
              <span style={{ color: account.color }} className="shrink-0">
                {account.name}
              </span>
              {toAccount && (
                <>
                  <span className="text-muted-foreground">→</span>
                  <span style={{ color: toAccount.color }} className="shrink-0">
                    {toAccount.name}
                  </span>
                </>
              )}
...
export default function Transactions() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterSubcategory !== "all")
      list = list.filter((t) => t.subcategory === filterSubcategory);
    if (filterAccount !== "all")
      list = list.filter(
        (t) => t.account === filterAccount || t.toAccount === filterAccount,
      );
    if (search.trim()) {
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3">
        <Card>
          <CardContent className="p-3 sm:pt-5">
            ...
            <p className="text-base sm:text-xl font-bold text-chart-1 truncate">
